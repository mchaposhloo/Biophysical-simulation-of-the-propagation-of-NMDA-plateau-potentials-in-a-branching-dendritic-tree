...
for counter in 1:39
    ...
    clamp_pos = terminal_node_4-counter;
    V_clamp_4th_cild = solve_linear(A_sparse,V,clamp_pos,40,0.01,100);
    pos[counter] = (terminal_node_4 - clamp_pos)*Δx;
...
plot(pos[:],amplitudes[7,:],seriestype=:scatter,markershape = :square,label = "soma")
plot!(pos[:],amplitudes[5,:],seriestype=:scatter,markershape = :circle,label = "1st child middle")
plot!(pos[:],amplitudes[4,:],seriestype=:scatter,markershape = :star,label = "2nd child middle")
plot!(pos[:],amplitudes[3,:],seriestype=:scatter,markershape = :hexagon,label = "4th child middle",legend = :topleft, legendfontsize =8,xaxis ="distance from the tip (cm)",yaxis ="voltage (mv)",ylim = (0,25))

savefig("location of synapse.svg")
