...
#the linear diameter function for the 2st branch
d4(x) = 0.000075#((d_L_branch_3_4 - d_0_branch_3_4)/L)*x + d_0_branch_3_4;
d5(x) = 0.000075#((d_L_branch_3_4 - d_0_branch_3_4)/L)*x + d_0_branch_3_4;
...
V_clamp_4th_cild = solve_linear(A_sparse,V,terminal_node_4-20,40,0.01,100);

t = 0:Δt:(M_t*Δt);

plot(t,V_clamp_4th_cild[200,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "back propagated signal")
plot!(t,V_clamp_4th_cild[162,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "forward propagated signal",legend = :topright, legendfontsize =10)
