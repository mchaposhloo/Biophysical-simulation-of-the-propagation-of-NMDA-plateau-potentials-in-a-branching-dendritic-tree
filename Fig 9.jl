...
V_clamp_2nd_cild = solve_linear(A_sparse,V,shared_node_2-20,40,0.01,100);

t = 0:Δt:(M_t*Δt);

plot(t,V_clamp_2nd_cild[180,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))

savefig("4th child middle.svg")

plot(t,V_clamp_2nd_cild[140,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))

savefig("3rd child middle.svg")

plot(t,V_clamp_2nd_cild[100,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))

savefig("2nd child middle.svg")

plot(t,V_clamp_2nd_cild[60,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))

savefig("1st child middle.svg")

plot(t,V_clamp_2nd_cild[20,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))

savefig("parent middle.svg")

plot(t,V_clamp_2nd_cild[1,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))
