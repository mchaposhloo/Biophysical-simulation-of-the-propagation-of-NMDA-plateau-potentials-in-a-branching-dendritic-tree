...
V_clamp_child_2_middle = solve_linear(A_sparse,V,terminal_node_4-1,40,0.01,100);

t = 0:Δt:(M_t*Δt);

plot(t,V_clamp_child_2_middle[180,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))

savefig("4th child middle.svg")

plot(t,V_clamp_child_2_middle[140,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))

savefig("3rd child middle.svg")

plot(t,V_clamp_child_2_middle[60,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))

savefig("1st child middle.svg")

plot(t,V_clamp_child_2_middle[20,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))

savefig("parent middle.svg")

plot(t,V_clamp_child_2_middle[1,:],title = "voltage over time",
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "",ylims = (0,35))
