...
function solve_linear_two_clamp_pos(A,U,clamp_pos_index_1,clamp_pos_index_2, vol_clamp_val, onset, dur)
    for t in 1:(size(U,2)-1)
        U[:,t+1] .= gmres(A,U[:,t],tol = 1e-5) #A\U[:,t]
        if (t>=(round(onset/Δt)+1)) && (t<=(round((onset+dur)/Δt)+1))
            U[clamp_pos_index_1,t+1] = vol_clamp_val
            U[clamp_pos_index_2,t+1] = vol_clamp_val
        end
    end
    return U
end
...
V_clamp_4th_3rd_childs = solve_linear_two_clamp_pos(A_sparse,V,terminal_node_4-1,terminal_node_3-1,40,0.01,100);

V = zeros(terminal_node_4,M_t+1);
V_clamp_4th_child = solve_linear(A_sparse,V,terminal_node_4-1, 40, 0.01, 100);

t = 0:Δt:(M_t*Δt);

plot(t,V_clamp_4th_child[100,:],
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "individual plateau",linestyle = :dot)
plot!(t,V_clamp_4th_3rd_childs[100,:],
    xaxis ="time (ms)",yaxis ="voltage (mv)", lw = 2.5,fmt = :svg,label = "two simultaneous plateaus", legendfontsize =9)

savefig("fig 17 left.svg")
...
savefig("fig 17 right.svg")

d_avg = zeros(1,35)
amplitudes = zeros(4,35);
...
for counter in 1:35
    ...
    amplitudes[1,counter] = maximum(V_clamp_4th_3rd_childs[1,:])
    amplitudes[2,counter] = maximum(V_clamp_4th_child[1,:])
    amplitudes[3,counter] = maximum(V_clamp_4th_3rd_childs[100,:])
    amplitudes[4,counter] = maximum(V_clamp_4th_child[100,:])
end

plot(d_avg[:],amplitudes[1,:],seriestype=:scatter,markershape = :square,label = "two simultaneous plateaus (soma)")
plot!(d_avg[:],amplitudes[2,:],seriestype=:scatter,markershape = :square,linecolor = :red,label = "individual plateau (soma)",legend = :topleft)
plot!(d_avg[:],amplitudes[3,:],seriestype=:scatter,markershape = :star4,label = "two simultaneous plateaus (2nd branch)")
plot!(d_avg[:],amplitudes[4,:],seriestype=:scatter,markershape = :star4,linecolor = :yellow,label = "individual plateau (2nd branch)",legend = :topleft,xaxis ="average diameter (cm)",yaxis ="voltage (mv)",ylims = (0,25),xlims = (0,0.0004))

savefig("and diameter.svg")

plot(d_avg[:], amplitudes[3,:]./amplitudes[4,:],seriestype=:scatter,markershape = :square,label = "2nd branch",xaxis ="average diameter (cm)",xlims = (0,0.0004))
plot!(d_avg[:], amplitudes[1,:]./amplitudes[2,:],seriestype=:scatter,markershape = :circle,label = "soma",xaxis ="average diameter (cm)",xlims = (0,0.0004))

savefig("fig 18 right.svg")
